...
def gantt_chart(
    dataset, dates, size_x=15, size_y=10, show_slack=True, time_label="Time"
):
    ...
    height = 1
    colors = [
        "#aec1d1" if dates.iloc[i, -1] > 0 else "#e6637d"
    ...
    for i in range(0, dates.shape[0]):
        ...
        ax.broken_barh(
            ...
            facecolors=colors_dict[tasks[i]],
        )
        ax.text(
            x=idx_e - (dur) / 2,
            y=yticks[i],
...
def gantt_chart_dep(
    ...
    plt.gca().axes.get_yaxis().set_visible(False)
    colors = [
        "#aec1d1" if dates.iloc[i, -1] > 0 else "#e6637d" for i in range(dates.shape[0])
    ]
    ...
    for i, task in enumerate(dataset):
        ...
        ax.broken_barh(
            [(es, dur)],
            (yticks[i] - height / 2, height),
            facecolors=color_dict[task_id],
        )
        ax.text(
            x=es + dur / 2, y=yticks[i], s=task_id, ha="center", va="center", color="k"
        )
...
def pert_method(dataset):
    ...
    idd = dict(zip(ids, idx))
    mean = [
        (dataset[i][2] + 4 * dataset[i][3] + dataset[i][4]) / 6
    ...
    while len(visited) < n:
        for i in reversed(range(n)):
            ...
            if task_id in visited:
                continue
            successors = [
                (succ_id, dep_type, lag, j)
                for j, (succ_id, succ_preds, _) in enumerate(dataset)
                for (pred_id, dep_type, lag) in succ_preds
                if pred_id == task_id
            ]
            if all(succ[0] in visited for succ in successors):
                ...
                visited.add(task_id)
    df = pd.DataFrame(
        dates, index=ids, columns=["ES", "EF", "LS", "LF", "Slack"]
    ).round(4)
    ...
    def trace_back(tid):
        for pid, dtyp, lag in preds_map.get(tid, []):
            if pid not in critical_set:
                ...
                if (
                    (dtyp == "FS" and np.isclose(c_es, p_ef + lag))
                    or (dtyp == "SS" and np.isclose(c_es, df.loc[pid, "ES"] + lag))
                    or (dtyp == "FF" and np.isclose(c_es + dur, p_ef + lag))
                    or (
    ...
    def trace_back(task_id):
        for pid, dtyp, lag in predecessors_map.get(task_id, []):
            if pid not in critical_set:
                ...
                ok = (
                    (dtyp == "FS" and np.isclose(c_es, p_ef + lag))
                    or (dtyp == "SS" and np.isclose(c_es, p_es + lag))
                    or (dtyp == "FF" and np.isclose(c_es + dur, p_ef + lag))
                    or (dtyp == "SF" and np.isclose(c_es + dur, p_es + lag))
                )
                if ok:
                    critical_set.add(pid)
                    trace_back(pid)
